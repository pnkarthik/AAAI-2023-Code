...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 25 12:24:33 2022
...
df = pd.read_csv('total-cost-varying-C-movielens.csv')
delta_vals = [np.exp(-1*i) for i in np.arange(1.0, 6.0, 2.0)]  # error probability
delta_vals = [int(np.log(1/delta)) for delta in delta_vals]

a = df['total-cost-C=0-FedElim0-movielens'].to_numpy(dtype = int)
b = df['total-cost-C=0-FedElim-movielens'].to_numpy(dtype = int)
c = df['total-cost-C=10-FedElim-movielens'].to_numpy(dtype = int)
d = df['total-cost-C=100-FedElim-movielens'].to_numpy(dtype = int)
err1 = df['error-bar-total-cost-C=0-FedElim0-movielens'].to_numpy(dtype = float)
err2 = df['error-bar-total-cost-C=0-FedElim-movielens'].to_numpy(dtype = float)
err3 = df['error-bar-total-cost-C=10-FedElim-movielens'].to_numpy(dtype = float)
err4 = df['error-bar-total-cost-C=100-FedElim-movielens'].to_numpy(dtype = float)
...
plt.show()
fig.savefig('total-cost-varying-C-movielens.pdf', dpi=300)

# # ----------------------------------------------------------------------------------------------------------'

df1 = pd.read_csv('FedElim-and-periodic-comm-cost-comparison-movielens.csv')
...
fig.set_figheight(40)
plt.show()
fig.savefig('comm-cost-comparison-movielens.pdf', dpi=300)

# ----------------------------------------------------------------------------------------------------------'

width = 0.13
df1 = pd.read_csv('FedElim-and-periodic-comm-total-cost-comparison-movielens.csv')
...
fig.set_figheight(40)
plt.show()
fig.savefig('FedElim-vs-periodic-total-cost-comparison-movielens.pdf', dpi=300)